...
# SBC
def calculate_reference_correlation(ref_counts, thresholds):
    # Assuming ref_counts is a list/array of reference count data
    ref_states_array = [tb.threshold(ref_counts[ind], thresholds[ind]) for ind in range(len(ref_counts))]
    flattened_ref_counts = [ref_states_array[ind].flatten() for ind in range(len(ref_counts))]
    ref_corr_coeffs = tb.nan_corr_coef(flattened_ref_counts)
    return ref_corr_coeffs


def threshold_counts(nv_list, sig_counts, ref_counts=None, dual_thresh_range=None, dynamic_thresh=True):
    """
    Threshold counts for NVs with specified thresholds in their sigs.
    If there's no threshold, the raw counts are just averaged as normal.
    """
    _validate_counts_structure(sig_counts)
    _validate_counts_structure(ref_counts)
    ...
    if dynamic_thresh:
        thresholds = [determine_threshold(sig_counts[ind], no_print=True) for ind in range(num_nvs)]
        print(f"Dynamic thresholds: {thresholds}")
    else:
        thresholds = [nv.threshold for nv in nv_list]
        print(f"Static thresholds: {thresholds}")

    thresholds = np.array(thresholds)[:, np.newaxis, np.newaxis, np.newaxis]

    def adjust_thresholds(thresholds, ref_counts, lr=0.1, beta=0.9, max_iter=100, tolerance=0.000001):
        """
        Optimizes the thresholds using gradient descent to minimize the pair correlation coefficient.
        """
        best_thresholds = thresholds.copy()
        best_ref_corr_coeffs = calculate_reference_correlation(ref_counts, best_thresholds)
        print(f"Initial thresholds: {np.ravel(thresholds)}")
        
        for i in range(max_iter):
            ref_corr_coeffs = calculate_reference_correlation(ref_counts, thresholds)
            
            # Find the indices of the maximum absolute correlation coefficient below the diagonal
            lower_triangle_indices = np.tril_indices_from(ref_corr_coeffs, k=-1)
            max_corr_idx = np.unravel_index(np.argmax(np.abs(ref_corr_coeffs[lower_triangle_indices])), ref_corr_coeffs.shape)
            max_corr_value = ref_corr_coeffs[max_corr_idx]
            
            # Check if the maximum correlation coefficient is below the tolerance
            if abs(max_corr_value) < tolerance:
                print(f"Optimization converged at iteration {i+1}")
                break
            
            # Adjust thresholds for the NVs corresponding to the maximum correlation coefficient
            nv_idx_1, nv_idx_2 = max_corr_idx
            
            # Initialize velocities and thresholds
            velocities = np.zeros_like(thresholds)
            velocities[nv_idx_1] = 0
            velocities[nv_idx_2] = 0

            for nv_idx in [nv_idx_1, nv_idx_2]:
                for direction in [-1, 1]:  # Both positive and negative adjustments
                    new_thresholds = thresholds.copy()
                    new_thresholds[nv_idx] += lr * direction
                    
                    new_ref_corr_coeffs = calculate_reference_correlation(ref_counts, new_thresholds)
                    
                    # Update velocities using momentum
                    gradients = np.max(new_ref_corr_coeffs, axis=0)
                    velocities[nv_idx] = beta * velocities[nv_idx] + (1 - beta) * gradients[nv_idx]
                    
                    # Update thresholds
                    thresholds[nv_idx] -= lr * velocities[nv_idx]
            
            thresholds = best_thresholds.copy()  # Update thresholds to the best found so far
            print(f"Iteration {i+1} max correlation coefficient at {max_corr_idx}: {max_corr_value}")
        
        return best_thresholds
    
    optimized_thresholds = adjust_thresholds(thresholds, ref_counts, lr=0.1, beta=0.9, max_iter=100, tolerance=0.000001)
    print(f"Optimized thresholds: {np.ravel(optimized_thresholds)}")
    
    if dual_thresh_range is None:
        sig_states_array = tb.threshold(sig_counts, optimized_thresholds)
    else:
        half_range = dual_thresh_range / 2
        sig_states_array = tb.dual_threshold(
            sig_counts, optimized_thresholds - half_range, optimized_thresholds + half_range
        )
    
    if ref_counts is not None:
        if dual_thresh_range is None:
            ref_states_array = tb.threshold(ref_counts, optimized_thresholds)
        else:
            ref_states_array = tb.dual_threshold(
                ref_counts, optimized_thresholds - half_range, optimized_thresholds + half_range
            )
    else:
...
#     else:
#         thresholds = [nv.threshold for nv in nv_list]
#         # thresholds = [22.87097125, 18.72903127, 14.71502195, 19.58658981, 25.20400837, 13.33880831, 9.24855832, 19.04139237, 12.65107179, 18.57274989]
